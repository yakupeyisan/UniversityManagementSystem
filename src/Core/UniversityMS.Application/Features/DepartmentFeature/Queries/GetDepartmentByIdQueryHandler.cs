...
public class GetDepartmentByIdQueryHandler : IRequestHandler<GetDepartmentByIdQuery, Result<DepartmentDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (department == null)
            {
                _logger.LogWarning("Department not found: {DepartmentId}", request.Id);
        ...
        catch (Exception ex)
        {
            ...
            return Result<DepartmentDto>.Failure("Bölüm bilgileri alınırken bir hata oluştu. Hata: "+ex.Message);
        }
    }
}