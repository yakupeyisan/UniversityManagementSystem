...
public class ActivateScheduleCommandHandler : IRequestHandler<ActivateScheduleCommand, Result>
{
    ...
    public async Task<Result> Handle(ActivateScheduleCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            _logger.LogInformation("Schedule activated. ScheduleId: {ScheduleId}", request.ScheduleId);
            return Result.Success("Program başarıyla aktif hale getirildi.");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error activating schedule");
            return Result.Failure("Program aktif hale getirilirken bir hata oluştu.", ex.Message);
        }
    }
}